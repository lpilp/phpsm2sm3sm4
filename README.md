...
### SM2签名常见问题
  ...
  * 同样的，在用本项目给***招行***签名的时候，就会出现上面同样的问题，编码int的时候，不能按标准的，认为签名的r,s都是32字节，不然对方就报错，这时对于偶发的小于32字节的rs，就得补0操作，src/util/FormatSign->format_cmbc()函数，目前只针对招行， 所以只是以补丁的方式，没有加入主程序
  ### 关于招行的国密的说明
  * 从招行给的公开sdk看，招行的国密库使用的java BC库，该库明明有签名返回的序列化的asn1(r,s)函数，程序员小哥哥又强制从asn1(r,s)取出r+s，然后又封装一个伪asn1函数，将r,s再次序列化，每个部份32字节，由于固定r,s长度为32字节，个别签名前面已经强制补了0，在伪asn1(r,s)序列化时，程序员小哥把它当成类似字符串处理，而实际上国密签名r,s是要当int处理，这就造成了这个补0的问题，在不该补0的时候补了0，当第三方调用的时候，使用标准asn1解析的时候，就报错了，需要进行兼容，或是忽略这个报错
  * 调用招行接口验签的问题，招行程序员小哥也是把这个签名当成字符串处理，将长度给固定了，取r,s的时候强制要求取32字节，这就出现个别的短签名长度不够，就直接报错了，兼容的做法是强制补0，补足32字节。
