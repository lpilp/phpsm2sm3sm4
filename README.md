...
### SM2签名常见问题
  ...
  * ***招行***的签名回调系统在做asn1编码的int类型的时候，标准是最高位大于7D是要补0，反之则不要补0，而招行返回的统一补0，使用的第三方phpasn1会报错，可以屏蔽该错误，或是将返回的签名过滤下，项目提供了一个过滤的类 src/util/FormatSign，使用测试见 test/tfsign.php
  * 同样的，在用本项目给***招行***签名的时候，就会出现上面同样的问题，编码int的时候，不能按标准的，无论高位是什么，都要强制补00，不然对方就报错, 目前的情况是这样，如果发现对方验证签名有问题的话，可以这种方法试试。
